...
def convert_episode(
    input_episode_dir: Path, episode_idx: int, output_dir: Path, fps: float, task_map: Dict[str, int] = None
) -> Dict[str, Any]:
    """转换单个episode"""
    ...
    return {"frames": frames_data, "length": episode_length, "episode_index": episode_idx, "task_info": task_info}


def create_videos_from_images(
    input_dir: Path, output_videos_dir: Path, episode_data: List[Dict], fps: float, code_type: str = "h264"
):
    """从图像序列创建视频文件，返回图像尺寸"""
    print("创建视频文件...")
...
def create_metadata_files(
    ...
    info = {
        ...
        "features": {
            ...
            "action": {"dtype": "float64", "shape": [action_dim], "names": [f"motor_{i}" for i in range(action_dim)]},
...
def main():
    ...
    parser.add_argument(
        "--filter_file", type=str, default="filter.txt", help="包含允许的episode编号列表的文件名 (相对于input_dir)"
    )
    ...
    if filter_file_path.exists():
        try:
            with open(filter_file_path, "r", encoding="utf-8") as f_filter:
                ...
                if not isinstance(filter_episode_numbers, list) or not all(
                    isinstance(n, int) for n in filter_episode_numbers
                ):
    ...
    create_metadata_files(
        episode_data, output_dir, args.dataset_name, args.robot_type, args.fps, image_shape, code_type=args.video_enc
    )
