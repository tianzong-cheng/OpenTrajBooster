...
def eagle_tensorrt_forward(self, vl_input):
    ...
    assert vl_input["pixel_values"].shape[0] <= 8, (
        "Batch size must be <= 8 because TensorRT engine was built with max_batch_size=8, you can try to adjust the"
        " max_batch_size in the build_engine.sh script and rebuild the engine."
    )
    ...
    embeddings = self.llm_engine(vl_input["input_ids"], vit_embeds, vl_input["attention_mask"])["embeddings"]
...
def action_head_tensorrt_forward(self, backbone_output, action_input):
    ...
    backbone_output.backbone_features = self.vlln_vl_self_attention_engine(backbone_output.backbone_features)["output"]
...
def setup_tensorrt_engines(policy, trt_engine_path):
    ...
    policy.model.action_head.action_encoder_engine = trt.Engine(os.path.join(trt_engine_path, "action_encoder.engine"))
    policy.model.action_head.action_decoder_engine = trt.Engine(os.path.join(trt_engine_path, "action_decoder.engine"))
    policy.model.action_head.DiT_engine = trt.Engine(os.path.join(trt_engine_path, "DiT.engine"))
    policy.model.action_head.state_encoder_engine = trt.Engine(os.path.join(trt_engine_path, "state_encoder.engine"))

    # Set TensorRT forward functions
    policy.model.backbone.forward = partial(eagle_tensorrt_forward, policy.model.backbone)
    policy.model.action_head.get_action = partial(action_head_tensorrt_forward, policy.model.action_head)
